...
uint256 hashGenesisBlock("0x7c96ab73849bb2e798aa2b3d340809b9d765f72c6e142c5e1363de740987dd5a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // XPDz: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "XPDz Signed Message:\n";
...
{
    // XPDz: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // XPDz: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // XPDz: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // 1 Oz of XPDz: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // XPDz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // XPDz: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // XPDz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me XPDz !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x0fe070d4e22f2dca93194d02c07df818b8d5716c35852f3d260b87cb3231c1863"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // XPDz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("XPDzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("XPDzMiner : ProcessBlock, block not accepted");
    }
